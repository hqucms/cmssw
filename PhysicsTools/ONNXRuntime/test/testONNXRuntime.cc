...
void testONNXRuntime::test(bool use_cuda) {
  std::string model_path = edm::FileInPath("PhysicsTools/ONNXRuntime/test/data/model.onnx").fullPath();
  auto session_options = ONNXRuntime::defaultSessionOptions(use_cuda);
...
void testONNXRuntime::checkCPU() { test(false); }
