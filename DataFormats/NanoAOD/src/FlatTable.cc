    ...
    for (unsigned int i = 0, n = other.nColumns(); i < n; ++i) {
        switch(other.columnType(i)) {
            ...
            case IntColumn:
                ...
                break;
	    case DoubleColumn:
                addColumn<double>(other.columnName(i), other.columnData<double>(i), other.columnDoc(i), other.columnType(i));
                break;
...
double nanoaod::FlatTable::getAnyValue(unsigned int row, unsigned int column) const {
    ...
    switch(columnType(column)) {
        ...
        case DoubleColumn:  return *(beginData<double>(column)+row);
