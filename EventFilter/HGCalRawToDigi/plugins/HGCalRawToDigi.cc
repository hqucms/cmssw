...
void HGCalRawToDigi::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup) {
  ...
  // unpackerConfig_.channelMax =
  //     std::get<0>(denseIdxMax) * std::get<1>(denseIdxMax) * std::get<2>(denseIdxMax) * std::get<3>(denseIdxMax) * 37;
  // unpackerConfig_.commonModeMax =
  //     std::get<0>(denseIdxMax) * std::get<1>(denseIdxMax) * std::get<2>(denseIdxMax) * std::get<3>(denseIdxMax) * 2;
  ...
  //   LogDebug("HGCalRawToDigi::erxbits") << it.first << " = "
  //                                       << "0x" << std::hex << (uint32_t)(it.second) << std::dec << std::endl;
}
