...
class BoostedJetTauONNXJetTagsProducer : public edm::stream::EDProducer<edm::GlobalCache<ONNXRuntime>> {
public:
  explicit BoostedJetTauONNXJetTagsProducer(const edm::ParameterSet &, const ONNXRuntime *);
  ~BoostedJetTauONNXJetTagsProducer() override;
...
private:
  typedef std::vector<reco::DeepBoostedTauJetTagInfo> TagInfoCollection;
  ...
  std::vector<float> center_norm_pad(const std::vector<float> &input,
                                     ...
                                     float min = 0,
                                     float max = -1);
  void make_inputs(const reco::DeepBoostedTauJetTagInfo &taginfo);
...
BoostedJetTauONNXJetTagsProducer::~BoostedJetTauONNXJetTagsProducer() {}

void BoostedJetTauONNXJetTagsProducer::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  // pfDeepBoostedTauJetTags
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("src", edm::InputTag("pfDeepBoostedTauJetTagInfos"));
...
std::unique_ptr<ONNXRuntime> BoostedJetTauONNXJetTagsProducer::initializeGlobalCache(const edm::ParameterSet &iConfig) {
  return std::make_unique<ONNXRuntime>(iConfig.getParameter<edm::FileInPath>("model_path").fullPath());
}

void BoostedJetTauONNXJetTagsProducer::globalEndJob(const ONNXRuntime *cache) {}

void BoostedJetTauONNXJetTagsProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
...
std::vector<float> BoostedJetTauONNXJetTagsProducer::center_norm_pad(const std::vector<float> &input,
                                                                  float center,
                                                                  float norm_factor,
                                                                  unsigned min_length,
                                                                  unsigned max_length,
...
void BoostedJetTauONNXJetTagsProducer::make_inputs(const reco::DeepBoostedTauJetTagInfo &taginfo) {
...
DEFINE_FWK_MODULE(BoostedJetTauONNXJetTagsProducer);
